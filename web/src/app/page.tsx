...
type MapTab = 'CUSTOM' | 'FR24' | 'ADSB';
type RightTab = 'ASSETS' | 'FR24' | 'ADSB';
...
export default function Dashboard() {
  ...
  return (
    <div style={{display:'flex',flexDirection:'column',height:'100vh',overflow:'hidden',background:C.bg,fontFamily:mono,color:C.text}}>
      ...
      {/* ═══ TOP BAR ═══ */}
      <div style={{display:'flex',alignItems:'center',height:42,borderBottom:`1px solid ${C.border}`,background:'#0a0e16',flexShrink:0,padding:'0 16px',gap:12}}>
        ...
        {[
          ...
          <div key={i} style={{display:'flex',alignItems:'center',gap:5,padding:'3px 8px',border:`1px solid ${C.border}`,background:'#0d1420'}}>
            <span style={{fontSize:7,color:C.dim,letterSpacing:1.5}}>{s.label}</span>
            <span style={{fontSize:12,color:s.color,fontWeight:700,minWidth:18,textAlign:'center'}}>{s.val}</span>
          </div>
        ))}
      ...
      <div style={{display:'flex',flex:1,overflow:'hidden'}}>
        ...
        <div style={{width:300,display:'flex',flexDirection:'column',borderRight:`1px solid ${C.border}`,background:C.panel,flexShrink:0}}>
          <div style={{display:'flex',borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
            {[['INTEL','INTEL'],['ALERTS','ALERTS'],['DEFENSE','DEFENSE'],['XFEED','X FEED']].map(([key,label]) => {
              const active = leftTab === key;
              return (
                <button key={key} className="tab"
          ...
          <div style={{flex:1,overflowY:'auto',padding:'6px'}}>
            {leftTab === 'INTEL' && filtered.map((item:any,i:number) => (
              <div key={`${item._id}-${i}`} className="ni" style={{marginBottom:5,padding:'9px 10px',border:`1px solid ${item.isMilitary?'rgba(255,51,51,0.35)':C.dimmer}`,borderLeft:`2px solid ${item.isMilitary?C.red:C.dimmer}`,background:item.isMilitary?'rgba(255,51,51,0.04)':'transparent',transition:'all 0.15s',animation:'fadeSlide 0.2s ease'}}>
            ...
            {leftTab === 'ALERTS' && (
              militaryAlerts.length > 0 ? militaryAlerts.map((f:any) => (
                <div key={f.id} className="ni" onClick={() => setSelectedFeature(f)} style={{marginBottom:5,padding:'9px 10px',border:`1px solid rgba(255,51,51,0.35)`,borderLeft:`2px solid ${C.red}`,background:'rgba(255,51,51,0.04)',animation:'fadeSlide 0.2s ease'}}>
                  <div style={{display:'flex',alignItems:'center',gap:5,marginBottom:5}}>
            ...
            {leftTab === 'DEFENSE' && defenseItems.map((item:any,i:number) => (
              <div key={i} className="ni" style={{marginBottom:5,padding:'9px 10px',border:`1px solid ${item.isHighPriority?'rgba(255,107,53,0.4)':C.dimmer}`,borderLeft:`2px solid ${item.isHighPriority?C.orange:C.dimmer}`,background:item.isHighPriority?'rgba(255,107,53,0.05)':'transparent',animation:'fadeSlide 0.2s ease'}}>
                <div style={{display:'flex',alignItems:'center',gap:5,marginBottom:5}}>
              ...
              <div>
                <div style={{display:'flex',gap:4,flexWrap:'wrap',marginBottom:10}}>
                  {X_ACCOUNTS.map((acc,i) => (
                    <button key={acc.handle} className="tab" onClick={() => setXAccount(i)}
        ...
        {/* ══ CENTER — MAP WITH TABS ══ */}
        <div style={{flex:1,display:'flex',flexDirection:'column',minWidth:0}}>

          {/* Map tab switcher */}
          <div style={{display:'flex',borderBottom:`1px solid ${C.border}`,background:'#0a0e16',flexShrink:0}}>
            {[
              {key:'CUSTOM',label:'🗺 INTEL MAP',desc:'News events + naval assets'},
              {key:'FR24',label:'✈ FLIGHTRADAR24',desc:'Live commercial + military flights'},
              {key:'ADSB',label:'📡 ADS-B EXCHANGE',desc:'Military coverage · squawk codes'},
            ].map(({key,label,desc}) => {
              const active = mapTab === key;
              return (
                <button key={key} className="tab"
                  onClick={() => setMapTab(key as MapTab)}
                  style={{padding:'8px 20px',fontSize:8,letterSpacing:1.5,fontFamily:mono,border:'none',cursor:'pointer',transition:'all 0.15s',textAlign:'left',
                  ...
                  <div style={{fontSize:7,color:active?'rgba(77,166,255,0.6)':C.dimmer,marginTop:2,letterSpacing:1}}>{desc}</div>
                </button>
              );
            })}

            {/* Overlays only shown on custom map */}
            {mapTab === 'CUSTOM' && (
              <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:6,padding:'0 12px'}}>
                <button onClick={() => setShowFlights(!showFlights)}
                  style={{padding:'4px 10px',fontSize:7,letterSpacing:2,fontFamily:mono,border:`1px solid ${showFlights?'rgba(77,166,255,0.6)':C.border}`,background:showFlights?'rgba(77,166,255,0.12)':'transparent',color:showFlights?C.blue:C.dim,cursor:'pointer',transition:'all 0.2s'}}>
                  ✈ [{flightData?.count||0}]
                </button>
                <button onClick={() => setShowShips(!showShips)}
                  style={{padding:'4px 10px',fontSize:7,letterSpacing:2,fontFamily:mono,border:`1px solid ${showShips?'rgba(0,212,170,0.6)':C.border}`,background:showShips?'rgba(0,212,170,0.12)':'transparent',color:showShips?C.teal:C.dim,cursor:'pointer',transition:'all 0.2s'}}>
                  ⚓ [{shipData?.count||0}]
                </button>
                {milFlights.length > 0 && (
            ...
            {mapTab === 'FR24' && (
              <div style={{marginLeft:'auto',display:'flex',alignItems:'center',padding:'0 12px'}}>
                <span style={{fontSize:7,color:C.dimmer,letterSpacing:1}}>CENTERED: UAE · GULF REGION · ZOOM 6</span>
              </div>
            )}
            ...
            {mapTab === 'ADSB' && (
              <div style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:10,padding:'0 12px'}}>
                ...
                <span style={{fontSize:7,color:C.orange,letterSpacing:1}}>MILITARY FILTER ACTIVE</span>
              </div>
            )}
          </div>

          {/* Map content — all same size, tab switches which is visible */}
          <div style={{flex:1,position:'relative'}}>

            {/* Custom Leaflet map */}
            <div style={{position:'absolute',inset:0,display:mapTab==='CUSTOM'?'block':'none'}}>
              {/* Legend */}
              <div style={{position:'absolute',bottom:16,left:10,zIndex:1000,background:'rgba(8,11,15,0.92)',border:`1px solid ${C.border}`,padding:'10px 14px',backdropFilter:'blur(4px)'}}>
            ...
            {/* FR24 iframe */}
            <div style={{position:'absolute',inset:0,display:mapTab==='FR24'?'block':'none'}}>
              <iframe
                src="https://www.flightradar24.com/simple_index.php?lat=24&lon=54&z=6&maptype=terrain"
                style={{width:'100%',height:'100%'}}
                title="Flightradar24 Live"
                allow="geolocation"
              />
            </div>

            {/* ADS-B Exchange iframe */}
            <div style={{position:'absolute',inset:0,display:mapTab==='ADSB'?'block':'none'}}>
              <iframe
        ...
        {/* ══ RIGHT PANEL ══ */}
        <div style={{width:300,display:'flex',flexDirection:'column',borderLeft:`1px solid ${C.border}`,background:C.panel,flexShrink:0}}>
          <div style={{padding:'10px 12px',borderBottom:`1px solid ${C.border}`,flexShrink:0}}>
            <div style={{fontSize:7,letterSpacing:3,color:C.dim,marginBottom:2}}>TRACKED ASSETS</div>
            <div style={{fontSize:8,color:C.dimmer}}>{(flightData?.count||0) + (shipData?.count||0)} TOTAL · {(flightData?.military||0) + (shipData?.military||0)} MILITARY</div>
          </div>
            ...
            <div style={{marginBottom:14}}>
              ...
              {!milFlights.length && (
                <div style={{padding:'10px 8px',fontSize:8,color:C.dimmer,letterSpacing:2,textAlign:'center'}}>
                  {flights.length > 0 ? `${flights.length} CIVILIAN AIRCRAFT IN REGION` : 'NO AIRCRAFT DATA'}
                </div>
              )}
            </div>
              ...
              {ships.filter((s:any) => s.isMilitary).map((s:any) => (
                <div key={s.mmsi} style={{marginBottom:4,padding:'8px 10px',border:`1px solid ${s.isCarrier?'rgba(181,123,238,0.4)':'rgba(0,212,170,0.25)'}`,borderLeft:`2px solid ${s.isCarrier?C.purple:C.teal}`,background:s.isCarrier?'rgba(181,123,238,0.05)':'transparent'}}>
                  <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:3}}>
                    ...
                    <span style={{fontSize:6,color:C.dimmer,letterSpacing:1,flexShrink:0,marginLeft:4}}>{s.isCarrier?'CVN':'WARSHIP'}</span>
                  </div>
      ...
      <div style={{height:26,borderTop:`1px solid ${C.border}`,background:'#0a0e16',flexShrink:0,display:'flex',alignItems:'center',overflow:'hidden'}}>
        ...
        <div style={{flex:1,overflow:'hidden',position:'relative'}}>
          <div style={{whiteSpace:'nowrap',animation:'ticker 40s linear infinite',fontSize:8,color:C.dim,letterSpacing:2,display:'inline-block'}}>
