...
const GIBS_LAYERS = [
  { id: null, name: '🗺️ Map Only' },
  { id: 'VIIRS_SNPP_CorrectedReflectance_TrueColor', name: '🌍 True Color (Daily)', url: 'https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/VIIRS_SNPP_CorrectedReflectance_TrueColor/default/{date}/GoogleMapsCompatible_Level9/{z}/{y}/{x}.jpg', maxZoom: 9 },
  { id: 'MODIS_Terra_CorrectedReflectance_Bands721', name: '🔥 Fire/False Color', url: 'https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/MODIS_Terra_CorrectedReflectance_Bands721/default/{date}/GoogleMapsCompatible_Level9/{z}/{y}/{x}.jpg', maxZoom: 9 },
  { id: 'VIIRS_SNPP_DayNightBand_ENCC', name: '🌙 Night Lights', url: 'https://gibs.earthdata.nasa.gov/wmts/epsg3857/best/VIIRS_SNPP_DayNightBand_ENCC/default/{date}/GoogleMapsCompatible_Level8/{z}/{y}/{x}.jpg', maxZoom: 8 },
];
...
export default function MapView({ features, selectedFeature, onFeatureSelect, militaryHighlight = true, showFlights = true, showShips = true }: Props) {
  ...
  const [selectedLayer, setSelectedLayer] = useState<any>(GIBS_LAYERS[0]);
  const [layerOpacity, setLayerOpacity] = useState(0.7);
  const [imageDate] = useState(() => {
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      features.forEach((f) => {
        ...
        const color = f.isMilitary ? '#dc2626' : { security: '#ea580c', politics: '#7c3aed', economy: '#059669' }[f.category as string] || '#2563eb';
  ...
  useEffect(() => {
    ...
    import('leaflet').then((L) => {
      ...
      flights.forEach((f: any) => {
        ...
        const color = f.isMilitary ? '#f59e0b' : '#60a5fa';
        const rotation = f.heading || 0;
        const icon = L.divIcon({
          className: '',
          html: `<div style="transform:rotate(${rotation}deg);font-size:${f.isMilitary ? '16' : '12'}px;filter:drop-shadow(0 1px 2px rgba(0,0,0,0.8))">✈️</div>`,
        ...
        marker.bindPopup(`
          <div style="font-family:system-ui;max-width:220px">
            ...
            <p style="margin:0;font-size:11px;color:#666">📡 ${Math.round((f.altitude || 0) / 30.48 * 100) / 100}ft · ${Math.round((f.velocity || 0) * 1.944)}kts</p>
      ...
      ships.forEach((s: any) => {
        ...
        marker.bindPopup(`
          <div style="font-family:system-ui;max-width:220px">
            ...
            <p style="margin:0;font-size:11px;color:#666">🏴 ${s.flag || 'Unknown'}</p>
            <p style="margin:0;font-size:11px;color:#666">⚡ ${s.speed?.toFixed(1) || 0} knots</p>
    ...
    <div className="relative w-full h-full">
      ...
      <div className="absolute top-4 right-4 z-[1000]">
        <button onClick={() => setShowLayerPanel(!showLayerPanel)}
          className="bg-gray-900 text-white px-3 py-2 rounded-lg text-sm font-medium shadow-lg border border-gray-700 flex items-center gap-2">
          🛰️ Layers {showLayerPanel ? '▲' : '▼'}
        </button>
        {showLayerPanel && (
          <div className="mt-2 bg-gray-900 border border-gray-700 rounded-xl shadow-2xl p-4 w-64">
            ...
            <div className="border-t border-gray-700 mt-3 pt-3 text-xs text-gray-500">
              Updated daily · Not live
            </div>
          </div>
        )}
      </div>
      ...
      <div className="absolute bottom-6 left-4 z-[1000] bg-gray-900/90 rounded-lg p-3 border border-gray-700">
        ...
        {[['🔴', 'Military event'], ['🟠', 'Security'], ['🔵', 'General news'], ['✈️', 'Aircraft'], ['🚢', 'Naval vessel'], ['🛸', 'Carrier']].map(([icon, label]) => (
